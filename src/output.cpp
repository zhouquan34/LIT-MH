...
template <typename T> 
void post_quantile(map<T, int>& v){
...
void mcmc_output(void){
	...
	vector<double> acr (3, 0);
	for (int i=0; i<3; i++){
		if (g_propose[i] == 0){acr[i] = 0;}
		else{ acr[i] =( (double) g_accept[i])/g_propose[i]; }
	}
...
*/
	...
	LOG << "Add = " << acr[0] << " (" << g_propose[0] << ")";
	LOG << "; Delete = " << acr[1] << " (" << g_propose[1] << ")";
	LOG << "; Long = " << acr[2] << " (" << g_propose[2] << ")";
	LOG << "\n" << endl;
