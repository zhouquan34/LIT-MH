...
class model{
	private:
		...
		double add_weight (int);
		double del_ll (int); // del_ll(k): deleting the k-th element in subset. 
		double del_weight (int);			

		// propose_add(log_proposal_prob): 
		//   return snp to add/delete sampled using informed weights with truncation
		//   set the value of log_proposal_prob 
		int propose_add(double&);
		int propose_del(double&, int = 0);

		// calculate log_proposal prob. 
		double calc_add(int);
		double calc_del(int, int = -1);
	...
	public:
		...
		// update_marginal(k) update g_pip, g_beta, g_model_size for k samples of this model
		...
		// add_snp(last_model): 
		//   call propose_add, initialize the current model using init_add. 
		//   return log proposal ratio (old - new). 
		// del_snp(last_model, if_last) check if the snp to be deleted is the last one (used for checking whether swap proposes same model)
		double add_snp(model*);
		double del_snp(model*); 
		double swap_snp(model*); 

		// compare_models(hastings_ratio, last_model):
		//   log_alpha = hastings_ratio + like - last_like. 
		//   return 1 if we accept the current one, and 0 if we accept last_model. 
		int compare_models(double, model*);
		
		// update g_rb_pip g_rb_beta 
		void rao_blackwell(void);

		int if_swap_allowed(void);
};
